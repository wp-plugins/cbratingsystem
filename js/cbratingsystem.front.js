jQuery(document).ready(function ($) {
	...
	$('.cbrp-content-container').each(function () {
		...
		jQuery('.cbrp-content-wprapper-form-' + ratingFormID + '-post-' + postID + ' .comment_box textarea').each(function () {
			...
			if (characters != 0) {
				$(resultSelector).append(my_string_prefix.string_prefix + " <strong>" + characters + "</strong> " + my_string_prefix.string_postfix);
				...
				$(this).keyup(function () {
					...
					if (resultSelector != false) {
						...
						$(resultSelector).html(my_string_prefix.string_prefix + " <strong>" + remaining + "</strong> " + my_string_prefix.string_postfix);
						if (remaining <= 10) {
		...
		$('.cbrp-content-wprapper-form-' + ratingFormID + '-post-' + postID + ' .cbrp-button-form-' + ratingFormID).click(function (event) {
			...
			jQuery('input.required').each(function () {
				...
				/*		var splitedNameAttr = nameAttr.replace('question', '').replace('[', '').replace(']', '').split('-');

						if (splitedNameAttr.length == 3) {
							//console.log(question[ratingFormID][q_id]);
							for (var i in question[ratingFormID][q_id]) {
								if ((i != inputType + '-' + q_id) && (i != q_id + '_type') && (question[ratingFormID][q_id][i] == 1)) {
									ansCount++;
								}
								count++;
							}
						...
						} else if (splitedNameAttr.length == 2) {
							if (question[ratingFormID][q_id][inputType + '-' + q_id] == 0) {
								jQuery('label[for="question-form-' + ratingFormID + '-q-' + q_id + '"]').addClass('error');
								questionError = true;
							} else {
								jQuery('label[for="question-form-' + ratingFormID + '-q-' + q_id + '"]').removeClass('error');
